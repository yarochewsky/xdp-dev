...
static __always_inline int vlan_pop(struct xdp_md* ctx, struct ethhdr* eth_hdr) {
	int vlan_id = -1;

	void* data_end = (void*) (long) ctx->data_end;
	void* data = (void*) (long) ctx->data;

	if (is_vlan(eth_hdr->h_proto)) {
		if (data + 1 > data_end) return -1;
		struct vlan_hdr* vlan = data + (int) sizeof(struct ethhdr);
		vlan_id = vlan->h_vlan_TCI;		
		struct ethhdr* eth_copy = data - (int) sizeof(struct vlan_hdr);
		if (bpf_xdp_adjust_head(ctx, (int) (sizeof(struct ethhdr) + sizeof(struct vlan_hdr)))) {
			return -1;
		}
			
		

		// [   eth   ]  [ vlan hdr ] [ payload ]
	}
